                ...
                if(!vibe_validate($quiz_dynamic)){
                    ...
                    if(is_array($questions) && is_array($questions['ques'])){
                      foreach($questions['ques'] as $m=>$question){
                        ...
                        $question_options = $this->fetch_cache($question.'_options');
                        if(empty($question_options)){
                          $question_options = get_post_meta($question,'vibe_question_options',true);
                          $this->record_cache($question.'_options',$question_options);
                        }
                      ...
                      $all_content = array();
                      foreach($questions['ques'] as $m=>$question){
                        $content = $wpdb->get_var($wpdb->prepare("SELECT comment_content FROM {$wpdb->comments} as c LEFT JOIN {$wpdb->commentmeta} as m ON c.comment_ID = m.comment_id WHERE m.meta_key = %s AND m.meta_value = %d AND c.comment_post_ID = %d AND c.user_id = %d AND c.comment_approved= %d",'quiz_id',$id,$question,$user_id,1));

                       
                        if(!empty($question_options) && is_array($question_options)){
                          $content = $question_options[($content-1)];
                        }
